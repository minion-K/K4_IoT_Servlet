...
* >> 클라이언트의 요청(Request)을 처리하고, 응답(Response)를 생성하는 역할
*
* cf) 웹 서버: 클라이언트 서버 구조에서 HTTP 요청을 받음
*       >> 정적인 응답에 대한 처리가 가능 (HTML 파일만 보기, pdf 파일만 보기)
*
...
*   - 사용자 요청에 동적 처리가 필요한 경우 톰캣이 해답 요청을 받음
*   - 톰캣이 자바 코드 실행 (Servlet | JSP 등)
*   - 결과로 HTML 등을 생성
*   - 웹 서버 또는 클라이언트에게 다시 전달
* 
* 2. 서블릿 동작 흐름
*   : 웹 브라우저가 서버로 HTTP 요청을 보낼 때, 서블릿의 동적 흐름
* 
*   [브라우저] - 사용자로부터 요청을 보냄(GET, POST, PUT, DELETE 등)
*   [웹 서버(Tomcat)] - 요청을 받아 해당 URL에 맞는 서블릿에 전달
*   [서블릿 컨테이너] - 서블릿을 관리하고, 생명주기(lifeCycle) 관리 , 요청/응답 객체 전달
*   [서블릿 클래스] - 실제로 요청을 처리하는 비즈니스 로직 담장자
*   [응답 반환] - 결과를 HTML/JSON 등으로 생성하여 브라우저로 반환
*
* 3. 서블릿 컨테이너(Tomcat)
*   : 서블릿 생성과 초기화, 서비스 요청 잔달과 종료의 모든 작업을 관리
